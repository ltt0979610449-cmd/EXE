...
public class BlogPostController {
    ...
    public BlogPostController(BlogPostService blogPostService, CloudinaryService cloudinaryService, ProvinceService provinceService, Validator validator) {
    ...
    @Operation(summary = "Tạo blog post mới", description = "Tạo blog post không kèm featured image")
    public ResponseEntity<ApiResponse<BlogPost>> createBlogPostJson(
            @RequestBody @Valid CreateBlogPostRequest request) {
        return createBlogPostInternal(request, null);
    }
    ...
    public ResponseEntity<ApiResponse<BlogPost>> createBlogPost(
            @RequestPart(value = "request", required = false) CreateBlogPostRequest requestPart,
            @ModelAttribute CreateBlogPostRequest formRequest,
        ...
        CreateBlogPostRequest request = resolveRequest(requestPart, formRequest);
    ...
    private CreateBlogPostRequest resolveRequest(CreateBlogPostRequest requestPart, CreateBlogPostRequest formRequest) {
        if (requestPart != null && !isEmpty(requestPart)) {
            return requestPart;
        }
        return formRequest;
    }

    private boolean isEmpty(CreateBlogPostRequest request) {
        if (request == null) {
            return true;
        }
        return isBlank(request.getTitle())
                && isBlank(request.getSlug())
                && isBlank(request.getContent())
                && request.getProvinceId() == null
                && isBlank(request.getBlocksJson());
    }

    private void validateRequest(CreateBlogPostRequest request) {
        for (ConstraintViolation<CreateBlogPostRequest> violation : validator.validate(request)) {
            throw new AppException(ErrorCode.REQUIRED_FIELD_MISSING, violation.getMessage());
        }
    }
}
