...
# Deploy backend
Write-Host "`nDeploy Spring Boot backend..." -ForegroundColor Yellow
fly deploy

Write-Host "`n✅ DEPLOYMENT HOÀN TẤT!" -ForegroundColor Green
Write-Host "`n📝 Thông tin deployment:" -ForegroundColor Cyan
Write-Host "   MySQL App: coiviet-mysql" -ForegroundColor White
Write-Host "   Backend App: coiviet-api" -ForegroundColor White
Write-Host "   Backend URL: $backendUrl" -ForegroundColor White
Write-Host "`n🔍 Kiểm tra status:" -ForegroundColor Cyan
Write-Host "   fly status -a coiviet-api" -ForegroundColor Gray
Write-Host "   fly logs -a coiviet-api" -ForegroundColor Gray
Write-Host "`n⚠️  LƯU Ý:" -ForegroundColor Yellow
Write-Host "   1. Cập nhật Google OAuth2 Redirect URI trong Google Console:" -ForegroundColor Yellow
Write-Host "      $backendUrl/login/oauth2/code/google" -ForegroundColor Gray
Write-Host "   2. Cập nhật MoMo Redirect URLs nếu cần" -ForegroundColor Yellow
Write-Host "   3. Kiểm tra health: $backendUrl/actuator/health" -ForegroundColor Yellow
